...
robot_file_paths = 'exp_data_final/0612_force_h18.csv'
vicon_file_paths = 'exp_data_final/0612_force_h18_vicon.csv'
...
# Force
ax = axs[0, 0]
ax.plot(time_vicon, -loader.vicon_force_z[0], label=r'Measured GRF (Vicon)', color=colors[0], linestyle='-', linewidth=linewidth)
...
ax = axs[1, 1]
ax.plot(time_vicon, -loader.vicon_force_z[2], label=r'Measured GRF (Vicon)', color=colors[0], linestyle='-', linewidth=linewidth)
...
for i in range(2):
    for j in range(2):
        ...
        axs[i, j].set_yticks(np.arange(0, 101, 20))
...
lines = [axs[0, 0].lines[0], Patch(facecolor=state_colors[2], edgecolor='none', alpha=0.3, label='Lower Rim'),
         axs[0, 0].lines[1], Patch(facecolor=state_colors[3], edgecolor='none', alpha=0.3, label='Foot Tip (Point G)'),
         axs[0, 0].lines[2]]
